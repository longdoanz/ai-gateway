...
class TestFullChatCompletionFlow:
    ...
    def test_full_flow_health_to_models_to_chat(self, test_client, valid_proxy_api_key):
        ...
        # NINE_ROUTER_URL is unconfigured in the test environment, so the
        # catalog is genuinely empty here — /v1/models sources it live from
        # 9router (the sole upstream) with no static fallback list anymore.
...
class TestModelsEndpointIntegration:
    ...
    def test_models_returns_all_available_models(self, test_client, valid_proxy_api_key):
        """
        What it does: Checks that the full 9router catalog is passed through.
        ...
        assert returned_ids == {"kiro/claude-sonnet-4", "openai/gpt-5"}
